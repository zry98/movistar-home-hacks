...
Download the script [`panel-controller/app.py`](https://raw.githubusercontent.com/zry98/movistar-home-hacks/main/IGW5000/panel-controller/app.py) and save it to `~/panel-controller/app.py`.

You can use `wget` with `wget https://raw.githubusercontent.com/zry98/movistar-home-hacks/main/IGW5000/panel-controller/app.py -O ~/panel-controller/app.py`.

Then, create the file `~/.config/systemd/user/panel-controller.service` with the following content:
...
And execute `systemctl --user daemon-reload && systemctl --user enable --now panel-controller.service` to make it run at startup.
